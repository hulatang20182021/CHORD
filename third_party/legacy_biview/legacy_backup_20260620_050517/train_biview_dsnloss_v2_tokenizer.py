...
def usage_entropy_loss(z, codebook, temperature=0.1):
    ...
    max_entropy = torch.log(torch.tensor(prob.shape[1], device=prob.device, dtype=prob.dtype))
    return 1.0 - entropy / (max_entropy + 1e-8)


def warmup(epoch, warmup_epochs):
    return min(1.0, epoch / max(int(warmup_epochs), 1))


def masked_cosine_distance(x, y, mask):
    if int(mask.sum()) == 0:
        return torch.zeros((), device=x.device)
    return cosine_distance(x[mask], y[mask])
...
class BiViewDSNLossTokenizerV2(BiViewDSNLossTokenizer):
    pass
...
def full_usage(model, st5, cf, device, batch_size=2048):
    ...
    c1, c2, c3 = [np.concatenate(x) for x in rows]
    p2 = np.stack([c1, c2], axis=1)
    p3 = np.stack([c1, c2, c3], axis=1)
    _, p3_counts = np.unique(p3, axis=0, return_counts=True)
    singleton = float((p3_counts == 1).sum() / len(p3_counts)) if len(p3_counts) else 0.0
    return {
        ...
        "p3_unique": int(len(np.unique(p3, axis=0))),
        "max_bucket_size_estimate": int(p3_counts.max()) if len(p3_counts) else 0,
        "prefix3_singleton_ratio_estimate": singleton,
    }
...
def main():
    ...
    p.add_argument("--candidate_epochs", default="1,3,5,10,20")
    ...
    p.add_argument("--lambda_usage_c1", type=float, default=0.01)
    p.add_argument("--lambda_usage_c2", type=float, default=0.01)
    p.add_argument("--lambda_usage_c3", type=float, default=0.05)
    p.add_argument("--usage_temperature", type=float, default=0.1)
    p.add_argument("--loss_warmup_epochs", type=int, default=5)
    p.add_argument("--difference_warmup_epochs", type=int, default=10)
    p.add_argument("--min_diff_batch_size", type=int, default=128)
    p.add_argument("--sem_private_cos_weight", type=float, default=1.0)
    p.add_argument("--early_stop_on_collapse", action=argparse.BooleanOptionalAction, default=True)
    p.add_argument("--invalid_patience", type=int, default=3)
    p.add_argument("--collapse_patience", type=int, default=2)
    p.add_argument("--allow_unconstrained_best", action=argparse.BooleanOptionalAction, default=False)
    ...
    effective_epochs = min(int(args.epochs), int(args.max_epochs))
    model = BiViewDSNLossTokenizerV2(st5.shape[1], cf.shape[1], args.latent_dim, args.codebook_size).to(device)
    ...
    config = {
        ...
        "lambda_usage_c1": args.lambda_usage_c1,
        "lambda_usage_c2": args.lambda_usage_c2,
        "lambda_usage_c3": args.lambda_usage_c3,
        ...
        "sem_private_cos_weight": args.sem_private_cos_weight,
        "early_stop_on_collapse": args.early_stop_on_collapse,
        "invalid_patience": args.invalid_patience,
        "collapse_patience": args.collapse_patience,
        "strict_gate_passed": False,
        "checkpoint_type": "strict_valid",
    }
    output.mkdir(parents=True, exist_ok=True)
    save_json(config, output / "config.json")

    best_valid_loss = float("inf")
    best_valid_structure_score = float("-inf")
    best_any_loss = float("inf")
    best_loss_epoch = None
    best_structure_epoch = None
    best_any_epoch = None
    ...
    for epoch in range(1, effective_epochs + 1):
        sums, seen = {}, 0
        model.train()
        loss_factor = warmup(epoch, args.loss_warmup_epochs)
        lambda_similarity_eff = args.lambda_similarity * loss_factor
        lambda_shared_eff = args.lambda_shared * loss_factor
        lambda_cf_private_eff = args.lambda_cf_private * loss_factor
        lambda_sem_private_eff = args.lambda_sem_private * loss_factor
        lambda_cf_delta_eff = args.lambda_cf_delta * loss_factor
        lambda_sem_delta_eff = args.lambda_sem_delta * loss_factor
        lambda_difference_eff = args.lambda_difference * warmup(epoch, args.difference_warmup_epochs)
        for batch in loader:
            ...
            sem_private_mse = F.mse_loss(out["semres"], sem_r)
            sem_private_cos = masked_cosine_distance(out["semres"], sem_r, sem_r.norm(dim=1) >= 1e-5)
            sem_private = sem_private_mse + args.sem_private_cos_weight * sem_private_cos
            ...
            usage_c1 = usage_entropy_loss(out["z_shared"], model.codebooks[0], args.usage_temperature)
            usage_c2 = usage_entropy_loss(out["z_cf_private"], model.codebooks[1], args.usage_temperature)
            usage_c3 = usage_entropy_loss(out["z_sem_private"], model.codebooks[2], args.usage_temperature)
            usage = (
                args.lambda_usage_c1 * usage_c1
                + args.lambda_usage_c2 * usage_c2
                + args.lambda_usage_c3 * usage_c3
            )
            loss = (
                args.lambda_cf_rec * cf_rec
                + args.lambda_sem_rec * sem_rec
                + out["vq"]
                + lambda_similarity_eff * similarity
                + lambda_difference_eff * difference
                + lambda_shared_eff * shared
                + lambda_cf_private_eff * cf_private
                + lambda_sem_private_eff * sem_private
                + lambda_cf_delta_eff * cf_delta
                + lambda_sem_delta_eff * sem_delta
            ...
            values = {
                ...
                "sem_private_mse": sem_private_mse,
                "sem_private_cos": sem_private_cos,
                "cf_delta": cf_delta,
                "sem_delta": sem_delta,
                "orth": orth,
                "var_loss": var,
        ...
        usage_stats = full_usage(model, st5, cf, device)
        row = {key: value / seen for key, value in sums.items()}
        row.update({
            "epoch": epoch,
            "lambda_similarity_eff": lambda_similarity_eff,
            "lambda_difference_eff": lambda_difference_eff,
            "lambda_shared_eff": lambda_shared_eff,
            "lambda_cf_private_eff": lambda_cf_private_eff,
            "lambda_sem_private_eff": lambda_sem_private_eff,
            "lambda_cf_delta_eff": lambda_cf_delta_eff,
            "lambda_sem_delta_eff": lambda_sem_delta_eff,
            **usage_stats,
        })
        row["strict_gate_passed"] = (
            row["c1_unique"] >= 60 and row["c2_unique"] >= 180 and row["c3_unique"] >= 180
        )
        row["structure_valid"] = row["strict_gate_passed"]
        row["structure_score"] = (
            row["p3_unique"]
            + 1000.0 * row.get("prefix3_singleton_ratio_estimate", 0.0)
            + 0.5 * row["c3_unique"]
            - 20.0 * row["max_bucket_size_estimate"]
        )
        history.append(row)
        print(json.dumps(row), flush=True)
        with (output / "training_log.jsonl").open("a", encoding="utf-8") as handle:
            handle.write(json.dumps(row) + "\n")
        save_checkpoint(output / f"checkpoint_epoch_{epoch}.pt", model, config, epoch, row["loss"])
        ...
        if row["structure_valid"] and row["loss"] < best_valid_loss:
            ...
            save_checkpoint(output / "best_loss_valid_model.pt", model, config, epoch, best_valid_loss)
            save_checkpoint(output / "best_valid_model.pt", model, config, epoch, best_valid_loss)
        if row["structure_valid"] and row["structure_score"] > best_valid_structure_score:
            best_valid_structure_score = row["structure_score"]
            best_structure_epoch = epoch
            save_checkpoint(output / "best_structure_model.pt", model, config, epoch, row["loss"])

        invalid_streak = 0 if row["strict_gate_passed"] else invalid_streak + 1
        tiny_streak = tiny_streak + 1 if (row["c1_unique"] <= 5 or row["c3_unique"] <= 5) else 0
        c3_dropping = (
            len(history) >= args.invalid_patience
            and all(history[-i]["c3_unique"] < history[-i - 1]["c3_unique"] for i in range(1, args.invalid_patience))
        )
        if args.early_stop_on_collapse:
            if tiny_streak >= args.collapse_patience:
                early_stop_reason = "c3_collapse"
                break
            if invalid_streak >= args.invalid_patience and c3_dropping:
                early_stop_reason = "invalid_patience"
                break

    save_checkpoint(output / "final_model.pt", model, config, history[-1]["epoch"], history[-1]["loss"])
    if best_structure_epoch is not None:
        (output / "best_model.pt").write_bytes((output / "best_structure_model.pt").read_bytes())
    elif args.allow_unconstrained_best:
        diag_config = dict(config)
        diag_config.update({"strict_gate_passed": False, "checkpoint_type": "unconstrained_diagnostic"})
        save_checkpoint(output / "best_model.pt", model, diag_config, best_any_epoch, best_any_loss)
    save_json({
        "history": history,
        "checkpoint_selection": "structure_first",
        "best_structure_epoch": best_structure_epoch,
        "best_structure_score": best_valid_structure_score if best_valid_structure_score > float("-inf") else None,
        "best_loss_valid_epoch": best_loss_epoch,
        "best_loss_valid": best_valid_loss if best_valid_loss < float("inf") else None,
        "best_structure_valid_loss": best_valid_loss if best_valid_loss < float("inf") else None,
        "best_structure_valid_epoch": best_structure_epoch,
        ...
        "strict_gate_passed": best_valid_loss < float("inf"),
        "early_stop_reason": early_stop_reason,
        "best_model_source": "best_structure_model.pt" if best_structure_epoch is not None else (
            "best_unconstrained_model.pt" if args.allow_unconstrained_best else None
        ),
        "checkpoint_type": "strict_valid" if best_structure_epoch is not None else (
            "unconstrained_diagnostic" if args.allow_unconstrained_best else "none"
        ),
