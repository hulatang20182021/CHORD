...
from project_paths import NEW_BASE, ST5_DIR, save_json  # noqa: E402
...
def default_base_dir(dataset, seed, compatibility_beauty):
    if compatibility_beauty and dataset == "Beauty":
...
def main():
    ...
    ap.add_argument("--compatibility_beauty", action="store_true", default=True)
    ap.add_argument("--no_compatibility_beauty", dest="compatibility_beauty", action="store_false")
    ap.add_argument("--force", action="store_true")
    args = ap.parse_args()

    dataset = args.dataset
    res = Path(args.resource_dir) if args.resource_dir else NEW_BASE / "results/resources" / dataset
    out = Path(args.output_dir) if args.output_dir else default_base_dir(dataset, args.seed, args.compatibility_beauty)
