...
mkdir -p "$PROJECT/results/chord/logs"
cd "$PROJECT"
...
for sd in $SHARED_DIMS; do
  gpu="${GPUS[$((i % ${#GPUS[@]}))]}"
  i=$((i + 1))
  log="$PROJECT/results/chord/logs/${DATASET}_${ORDER}_sd${sd}_seed${SEED}_sweep.log"
  ...
  BEAM_SIZE="$BEAM_SIZE" \
  FORCE="$FORCE" \
    bash scripts/run_chord_pipeline.sh 2>&1 | tee "$log"
done

/home/huangxin/miniconda3/envs/emotion_ml1m/bin/python scripts/collect_chord_report.py --dataset "$DATASET" --seed "$SEED"
