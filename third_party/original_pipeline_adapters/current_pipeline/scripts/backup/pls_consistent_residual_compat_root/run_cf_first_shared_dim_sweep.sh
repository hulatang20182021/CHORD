...
for sd in $SHARED_DIMS; do
  ...
  BEAM_SIZE="$BEAM_SIZE" \
  FORCE="$FORCE" \
    bash scripts/run_pls_consistent_pipeline.sh 2>&1 | tee "$log"
done

/home/huangxin/miniconda3/envs/emotion_ml1m/bin/python scripts/collect_pls_consistent_report.py --dataset "$DATASET" --seed "$SEED"
