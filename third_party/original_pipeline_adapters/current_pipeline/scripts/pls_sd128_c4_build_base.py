...
def pca_l2(x, dim, seed):
    x = scaled(x)
    z = PCA(n_components=min(dim, x.shape[1], x.shape[0] - 1), random_state=seed, svd_solver="randomized").fit_transform(x)
...
def main():
    ...
    out = C4_BASE / "base/pls_sd128_base"
    ...
    raw = {str(i): {"item_id": item_order[i], "c1": int(c1[i]), "c2": int(c2[i]), "c3": int(c3[i])} for i in range(len(item_order))}
    summary = {
    ...
    for name, arr in [("z_shared", z_shared), ("z_cfres", z_cfres), ("z_semres", z_semres), ("c1", c1), ("c2", c2), ("c3", c3), ("kmeans_c1_centers", center1), ("kmeans_c2_centers", center2), ("kmeans_c3_centers", center3)]:
