...
def main():
    ...
    for order in [x for x in args.orders.split(',') if x]:
        ...
        run([
            CONDA,'run','--no-capture-output','-n','emotion_ml1m','python', PROJECT/'scripts/cold_start/export_cold_start_warm_beams.py',
        ...
        for prefix_len in [int(x) for x in args.prefix_lens.split(',') if x]:
            ...
            run([
                CONDA,'run','--no-capture-output','-n','emotion_ml1m','python', PROJECT/'scripts/cold_start/eval_cold_start_tiger_style.py',
