...
for dataset in $DATASETS; do
  for seed in $SEEDS; do
    ...
    "$PY" scripts/pls_consistent_residual/generate_pls_consistent_index.py \
    ...
    "$PY" scripts/pls_consistent_residual/audit_pls_consistent_index.py \
      --dataset "$dataset" --seed "$seed" --order "$ORDER" \
      --shared_dim "$SHARED_DIM" --codebook_size "$CODEBOOK_SIZE" \
    ...
    "$PY" scripts/pls_consistent_residual/run_one_pls_consistent_downstream.py \
      --dataset "$dataset" --seed "$seed" --order "$ORDER" \
      --shared_dim "$SHARED_DIM" --codebook_size "$CODEBOOK_SIZE" \
...
"$PY" scripts/pls_consistent_residual/collect_pls_consistent_report.py
