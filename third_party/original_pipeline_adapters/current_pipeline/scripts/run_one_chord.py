...
def ensure_static(dataset, seed, env, dry_run=False):
    ...
    static_run = f"{dataset}_chord_seed{seed}"
    ...
    if not (index_dir / f"{static_run}.index.json").exists():
        ...
        execute(cmd, RESULT_BASE / "logs" / f"{dataset}_chord_index_seed{seed}.build.log", env, cwd=NEW_BASE, dry_run=dry_run, stage=f"{dataset}:static_chord_index")
...
def main():
    parser = argparse.ArgumentParser(description="Run CHORD downstream.")
    ...
    parser.add_argument("--print_every", type=int, default=int(os.environ.get("PRINT_EVERY", "50")))
    ...
    parser.add_argument("--wandb_project", default=os.environ.get("WANDB_PROJECT", "chord"))
    ...
    if args.dry_run:
        print("[dry_run] stopped after resource/static command planning; no files were written by child commands.")
        return
    paths = validate_inputs(args.dataset, args.seed)

    run_name = (
        f"{args.dataset}_chord_seed{args.seed}"
