...
def prep_r4(base):
    ...
    cen1, cen2, cen3 = (
        np.load(base / "kmeans_c1_centers.npy"),
        np.load(base / "kmeans_c2_centers.npy"),
        np.load(base / "kmeans_c3_centers.npy"),
    )
...
def assign_global(g, r4, k, seed, center_path):
...
def emit(dataset, run_name, variant, c4, meta, c1, c2, c3, item_order, old_summary):
    ...
    summary = {
        ...
        "run_name": run_name,
        "c4_variant": variant,
        "p3_unique": old_summary["p3_unique"],
        "max_bucket_size": old_summary["max_bucket_size"],
        "old_max_c4": old_summary["max_c4"],
        "max_c4": old_summary["max_c4"],
        "prefix3_singleton_ratio": old_summary["prefix3_singleton_ratio"],
        "new_c4_unique": int(len(np.unique(c4))),
        "duplicate_sid_count": dup,
        "c4_entropy": float(entropy(usage + 1e-12, base=2)),
        "c4_usage_min": int(nonzero.min()) if len(nonzero) else 0,
        "c4_usage_max": int(nonzero.max()) if len(nonzero) else 0,
...
def main():
    ...
    base = C4_BASE / "base" / f"{args.dataset}_pls_sd128_base_seed{args.seed}"
    ...
    g = groups(c1, c2, c3)
    specs = [
        ("dpos_baseline", f"{args.dataset}_plssd128_c4_dpos_baseline_seed{args.seed}", assign_dpos(g, item_order)),
        ("residual_sort", f"{args.dataset}_plssd128_c4_residual_sort_seed{args.seed}", assign_sort(g, item_order, r4, args.seed)),
    ]
    for k in [64, 128, 256]:
        center_path = base / f"c4_global_k{k}_centers.npy"
        specs.append((f"global_residual_k{k}_assign", f"{args.dataset}_plssd128_c4_globalres_k{k}_seed{args.seed}", assign_global(g, r4, k, args.seed + k, center_path)))
