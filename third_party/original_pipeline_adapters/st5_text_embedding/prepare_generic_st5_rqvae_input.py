...
from project_paths import ROOT, ST5_MODEL
...
def ensure_no_existing(paths: list[Path]) -> None:
    existing = [str(path) for path in paths if path.exists() and path.stat().st_size > 0]
    if existing:
        raise SystemExit("Refusing to overwrite existing non-empty outputs:\n" + "\n".join(existing))
...
def main() -> None:
    ...
    parser.add_argument("--model_path", default=str(ST5_MODEL))
    ...
    index_path = data_dir / f"{args.dataset}.index.json"
    ...
    report = BASE / f"results/reports/{args.dataset}_st5_rqvae_input_report.md"
    ensure_no_existing([out_emb, out_order, out_summary, report])

    index = {str(k): v for k, v in load_json(index_path).items()}
    item = {str(k): v for k, v in load_json(item_path).items()}
    ...
    summary = {
        ...
        "source": f"data/{args.dataset}/{args.dataset}.item.json",
