...
def main() -> None:
    ...
    index_path = out / "Beauty_plain_st5_rqvae.index.json"
    raw_path = out / "Beauty_plain_st5_rqvae_raw_codes.json"
    summary_path = out / "Beauty_plain_st5_rqvae_build_summary.json"
    ...
    model = PlainResidualVQVAE(**ckpt["config"]).to(args.device)
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()
    with torch.no_grad():
        _, _, indices = model(torch.from_numpy(emb).to(args.device))
