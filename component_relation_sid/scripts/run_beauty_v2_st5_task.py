...
def main() -> int:
    ...
    status = {
        "dataset": DATASET,
        "method": "component_relation_sid_v2_st5",
        "seed": args.seed,
        "target_epochs": 20,
        "gpu_id": args.gpu_id,
        ...
        "notes": "Fair Beauty 20-epoch run; ST5 T5EncoderModel mean-pooling fallback; early stopping patience runtime-overridden to 1000",
    }
    save_json(status, run_dir / "status.json")
    env = os.environ.copy()
    env["CUDA_VISIBLE_DEVICES"] = str(args.gpu_id)
    env["WANDB_DISABLED"] = "true"
    with (run_dir / "train_stdout.log").open("w", encoding="utf-8") as stdout, (run_dir / "train_stderr.log").open("w", encoding="utf-8") as stderr:
    ...
    with (run_dir / "eval_stdout.log").open("w", encoding="utf-8") as stdout, (run_dir / "eval_stderr.log").open("w", encoding="utf-8") as stderr:
        result = subprocess.run(eval_cmd, cwd=TIGER, env=env, stdout=stdout, stderr=stderr)
    status.update({"status": "completed" if result.returncode == 0 else "eval_failed", "eval_returncode": result.returncode, "finished_at": now()})
    save_json(status, run_dir / "status.json")
    return result.returncode
