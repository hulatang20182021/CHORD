...
DATA="${ROOT}/data/Beauty_component_relation_sid_v2_llama"
RUN_DIR="${BASE}/results/downstream_20epoch/beauty_component_relation_sid_v2_llama_seed${SEED}"
CHECKPOINT_DIR="${BASE}/checkpoints/Beauty/component_relation_sid_v2_llama_seed${SEED}"
REPORT_DIR="${BASE}/results/reports"
LOG="${REPORT_DIR}/beauty_component_relation_sid_v2_llama_20epoch_seed${SEED}.log"
MARKER="${REPORT_DIR}/experiments_snapshot_v2_llama_20epoch_marker"

cd "${ROOT}"
for file in \
  "${DATA}/Beauty_component_relation_sid_v2_llama.index.json" \
  "${DATA}/Beauty_component_relation_sid_v2_llama.inter.json" \
  "${DATA}/Beauty_component_relation_sid_v2_llama.item.json"; do
  if [[ ! -s "${file}" ]]; then
...
if [[ "${GPU_USED}" -gt 256 ]]; then
  ...
  if [[ "${GPU_ID}" == "2" ]]; then
    echo "[NEXT] GPU2 is busy. Re-run with GPU3: bash component_relation_sid/scripts/run_beauty_v2_llama_20epoch.sh ${CONDA_ENV} 3 ${SEED}" >&2
  fi
  exit 4
fi
...
{
  echo "[START] $(date -Is)"
  echo "[DATASET] Beauty_component_relation_sid_v2_llama"
  echo "[ENCODER LIMIT] exploratory Llama hidden-state mean pooling; not TIGER Sentence-T5"
  ...
  CUDA_VISIBLE_DEVICES="${GPU_ID}" /home/huangxin/anaconda3/bin/conda run -n "${CONDA_ENV}" python \
    "${BASE}/scripts/run_beauty_v2_llama_task.py" \
    --conda_env "${CONDA_ENV}" \
    --gpu_id "${GPU_ID}" \
    --seed "${SEED}"
