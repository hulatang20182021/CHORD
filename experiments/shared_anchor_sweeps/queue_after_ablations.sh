...
for dataset in Beauty Instruments Yelp; do
  echo "[$(date --iso-8601=seconds)] start $dataset direct70 sweep50-70 step5" | tee -a "$LOG"
  bash "$PROJECT/experiments/shared_anchor_sweeps/run_dataset_direct70_sweep50_70_step5.sh" "$dataset" \
    2>&1 | tee -a "$LOG"
done
