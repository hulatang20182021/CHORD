...
RUN_NAME=Beauty_k1024_concat_generic_aligned_supervision_seed42_fixed60_v1
...
"$PY" "$PROJECT/experiments/shared_anchor_ablations/static_intersection_downstream_finetune_shared_anchor_ablation.py" \
  ...
  --pcsc_ablation_variant a1_same --pcsc_aux \
...
manifest = {
    ...
    "variant": "concat_no_decomp_with_generic_aligned_block_prefix_supervision",
    "interpretation": "role-neutral block-aligned supervision control; not semantic/CF PCSC",
    "routing": {
        "anchor": "h1 -> block1",
        "aligned_path_2": ["h2 -> block2", "h1+h2 -> prefix12"],
        "aligned_path_3": ["h3 -> block3", "h1+h3 -> prefix13"],
    },
    "supervision_contract": json.loads(
        (Path(os.environ["TARGETS"]) / "manifest.json").read_text()
    ),
    "downstream_objective": "SID-CE + five equally weighted generic aligned cosine losses",
...
echo "complete K1024 Beauty concat generic aligned-supervision control"
