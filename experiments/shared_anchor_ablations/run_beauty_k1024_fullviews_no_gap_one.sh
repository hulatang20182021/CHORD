...
MODE=${1:?usage: $0 ceonly|crossview}
case "$MODE" in
  ceonly)
    PCSC_VARIANT=a0_ceonly
    ;;
  crossview)
    PCSC_VARIANT=a7_main
    ;;
  *)
...
INDEX_NAME=Beauty_chord_seed42_pls_fullsem_fullcf_no_gap_k1024
BASE=$ROOT/base/$INDEX_NAME
INDEX=$ROOT/index/$INDEX_NAME/$INDEX_NAME.index.json
RES=$ROOT/resources/$RESOURCE
RUN_NAME=Beauty_k1024_pls_fullviews_no_gap_${MODE}_seed42_fixed60_v1
...
"$PY" "$PROJECT/scripts/build_beauty_pls_fullviews_no_gap_control.py" \
  --result_base "$ROOT"
...
pcsc_flags=()
[[ "$MODE" == ceonly ]] || pcsc_flags+=(--pcsc_aux)
"$PY" "$PROJECT/experiments/shared_anchor_ablations/static_intersection_downstream_finetune_shared_anchor_ablation.py" \
  ...
  --shared_emb "$ROOT/base/$SOURCE/z_shared.npy" \
  ...
  --pcsc_ablation_variant "$PCSC_VARIANT" "${pcsc_flags[@]}" \
...
manifest = {
    ...
    "variant": f"PLS_consensus_plus_full_views_no_gap_{mode}",
    "tokenizer": json.loads(
        (Path(os.environ["BASE"]) / "base_build_summary.json").read_text()
    ),
    "downstream_objective": (
        "SID-CE only" if mode == "ceonly"
        else "SID-CE + exact five-term shared-anchor cross-view PCSC"
    ),
...
echo "complete K1024 Beauty full-view no-gap mode=$MODE"
