...
ALLOW_UNCONSTRAINED_BEST=${ALLOW_UNCONSTRAINED_BEST:-0}
...
echo "============================================================"
echo "Bi-view DSN-loss v2 SID Tokenizer"
...
echo "ALLOW_UNCONSTRAINED_BEST=$ALLOW_UNCONSTRAINED_BEST"
echo "============================================================"
...
"$PYTHON" "$NEW_BASE/scripts/train_biview_dsnloss_v2_tokenizer.py" \
  --dataset "$DATASET" \
  ...
  --seed "$SEED" \
  --epochs "$TOK_EPOCHS" \
  --max_epochs "$TOK_EPOCHS" \
  $(if [[ "$ALLOW_UNCONSTRAINED_BEST" == "1" ]]; then echo "--allow_unconstrained_best"; fi) \
  --device "$DEVICE" \
...
"$PYTHON" "$NEW_BASE/scripts/generate_biview_dsnloss_v2_index.py" \
