...
from train_biview_shared_private_tokenizer import BiViewSharedPrivateTokenizer
...
def main():
    ...
    if all(x.exists() for x in outputs):
        print(f"SKIP existing complete index: {out_dir}")
        return
    if any(x.exists() for x in outputs):
        raise SystemExit(f"Partial index exists; refusing overwrite: {out_dir}")

    ckpt = torch.load(args.checkpoint, map_location="cpu", weights_only=False)
    cfg = ckpt["config"]
    model = BiViewSharedPrivateTokenizer(
        cfg["input_dim_sem"], cfg["input_dim_cf"], cfg["latent_dim"], cfg["codebook_size"]
    )
    ...
    order = [str(x) for x in load_json(args.item_order)]
    ...
    for bucket, items in buckets.items():
        for pos, item in enumerate(sorted(items, key=lambda x: int(x) if str(x).isdigit() else str(x))):
            ...
            index[item] = sid
    sid_counts = Counter(tuple(v) for v in index.values())
    duplicate = sum(v - 1 for v in sid_counts.values() if v > 1)
    if duplicate:
        raise ValueError(f"Duplicate SID count: {duplicate}")
    p2 = Counter(tuple(s[:2]) for s in index.values())
    p3 = Counter(tuple(s[:3]) for s in index.values())
    summary = {
        "num_items": len(index),
        "duplicate_sid_count": duplicate,
        "unique_sid_count": len(sid_counts),
        "max_c4": max(v["c4"] for v in raw_codes.values()) if raw_codes else 0,
        "c1_unique": len({v["c1"] for v in raw_codes.values()}),
        "c2_unique": len({v["c2"] for v in raw_codes.values()}),
        "c3_unique": len({v["c3"] for v in raw_codes.values()}),
        "c4_unique": len({v["c4"] for v in raw_codes.values()}),
        "p2_unique": len(p2),
        "p3_unique": len(p3),
        "prefix3_singleton_ratio": sum(v == 1 for v in p3.values()) / len(p3) if p3 else 0.0,
    }
