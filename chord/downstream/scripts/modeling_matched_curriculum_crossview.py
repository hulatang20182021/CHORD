...
class MatchedCurriculumLETTER(LETTER):
    ...
    def _pcsc_loss(self, hidden):
        ...
        lengths = torch.tensor(
            [record[0].numel() for record in records], device=hidden.device, dtype=torch.long
        )
        starts = torch.repeat_interleave(
            torch.tensor([record[1] for record in records], device=hidden.device, dtype=torch.long),
            lengths,
        )
