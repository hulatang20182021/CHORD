...
def main():
    ...
    run_name = f"{args.dataset}_chord_static_{args.order}_sd{args.shared_dim}_k{args.codebook_size}_seed{args.seed}_down{args.epochs}_beam{args.num_beams}{suffix}"
    ...
    execute([
        ...
        PROJECT / "scripts/chord_static_sweep/build_chord_downstream_data.py",
    ...
    train_cmd = [
        CONDA, "run", "--no-capture-output", "-n", "emotion_ml1m", "python",
        PROJECT / "scripts/chord_static_sweep/finetune_chord.py",
        "--output_dir", ckpt,
        "--dataset", alias,
    ...
    audit_path = RESULT_BASE / "reports" / f"{args.dataset}_{args.order}_sd{args.shared_dim}_k{args.codebook_size}_seed{args.seed}_static_audit.json"
