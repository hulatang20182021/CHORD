...
def main():
    ...
    env["PYTHONPATH"] = os.pathsep.join([str(PROJECT / "scripts"), str(PROJECT / "scripts/pls_consistent_residual"), str(TIGER)])

    alias = run_name
    data_dir = RESULT_BASE / "data" / alias
    execute([
        CONDA, "run", "--no-capture-output", "-n", "emotion_ml1m", "python",
        PROJECT / "scripts/build_pls_consistent_downstream_data.py",
    ...
    execute([
        CONDA, "run", "--no-capture-output", "-n", "emotion_ml1m", "python",
        PROJECT / "scripts/finetune_pls_consistent.py",
        "--output_dir", ckpt,
        "--dataset", alias,
