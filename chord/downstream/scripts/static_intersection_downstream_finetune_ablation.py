...
from modeling_matched_curriculum_letter_ablation import MatchedCurriculumLETTER
from utils import (
    ensure_dir,
    load_datasets,
    parse_dataset_args,
...
def main(args):
    ...
    if args.resume_from_checkpoint and args.pcsc_alignment == "positional":
        ...
        if existing is None and not args.allow_unversioned_legacy_resume:
            raise RuntimeError(
                "Refusing to resume positional legacy PCSC from an unversioned checkpoint. "
                "Pass --allow_unversioned_legacy_resume only for an audited legacy run."
            )
