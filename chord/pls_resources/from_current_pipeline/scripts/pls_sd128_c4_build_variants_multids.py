...
def main():
    ...
    g = groups(c1, c2, c3)
    specs = [
        ("dpos_baseline", f"{args.dataset}_chord_seed{args.seed}", assign_dpos(g, item_order)),
        ("residual_sort", f"{args.dataset}_chord_residual_sort_seed{args.seed}", assign_sort(g, item_order, r4, args.seed)),
    ]
    for k in [64, 128, 256]:
        center_path = base / f"c4_global_k{k}_centers.npy"
        specs.append((f"global_residual_k{k}_assign", f"{args.dataset}_chord_globalres_k{k}_seed{args.seed}", assign_global(g, r4, k, args.seed + k, center_path)))
