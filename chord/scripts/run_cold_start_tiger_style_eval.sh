...
$PY $PROJECT/scripts/cold_start_build_tiger_style_assets.py --dataset $DATASET --cold_ratio $COLD_RATIO --seed $SEED --cold_seed $COLD_SEED --prefix_len 3 --force
warm_index=$COLD_BASE/$split_key/tiger_style/warm_only.index.json
echo "[stage] export warm-only beams"
CUDA_VISIBLE_DEVICES=$GPU $PY $PROJECT/scripts/export_cold_start_warm_beams.py --dataset $DATASET --split_key $split_key --checkpoint $checkpoint --warm_index $warm_index --eval_alias $eval_alias --data_root $COLD_BASE/data --num_beams $EXPORT_BEAMS --test_batch_size $TEST_BATCH_SIZE --gpu $GPU --output $out_dir/warm_beams.jsonl --seed $SEED
IFS=',' read -ra PFX <<< "$PREFIX_LENS"
for p in "${PFX[@]}"; do
  echo "[stage] eval prefix$p"
  $PY $PROJECT/scripts/eval_cold_start_tiger_style.py --beams_jsonl $out_dir/warm_beams.jsonl --cold_prefix_map $COLD_BASE/$split_key/tiger_style/cold_prefix${p}_to_items.json --cold_item_to_sid $COLD_BASE/$split_key/tiger_style/cold_item_to_sid.json --warm_items $COLD_BASE/$split_key/warm_items.json --cold_items $COLD_BASE/$split_key/cold_items.json --k_list 1,5,10 --epsilons $EPSILONS --prefix_len $p --output_metrics $out_dir/metrics_prefix${p}.json --output_details $out_dir/details_prefix${p}.jsonl
done
echo "[stage] collect report"
$PY $PROJECT/scripts/collect_cold_start_tiger_style_report.py --eval_dir $out_dir --train_run $train_run
echo "[done] $out_dir"
